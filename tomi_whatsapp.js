...
// Версия 3.5 — геолокация при открытии + новые карточки предоплат
// ══════════════════════════════════════════════════════════════════════
...
const conversations = {};
const openShifts = {};
const pendingGeoAction = {}; // { userId: 'open_shift' | 'close_shift' }
...
// ── Форматирование карточки предоплаты (новый дизайн) ─────────────────
function formatPrepayCard(p, num) {
  const isClosed = p.status.includes('закрыт');
  const initials = p.client.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
  const total = (p.amount || 0) + (p.balance || 0);
  const statusLabel = isClosed ? '✅ Закрыта' : '🟡 Открыта';

  let card = '─────────────────────\n';
  card += statusLabel + '  №' + num + '\n';
  card += '👤 ' + p.client + '\n';
  if (p.id) card += '🆔 ' + p.id + ' · ' + p.date + '\n';
  if (p.phone) card += '📞 ' + p.phone + '\n';
  if (p.items && p.items.length) card += '👗 ' + p.items.join(', ') + '\n';
  card += '\n';
  if (total > 0) card += '💵 Цена товара:   ' + Number(total).toLocaleString() + ' тг\n';
  card += '💰 Аванс:         ' + Number(p.amount).toLocaleString() + ' тг\n';
  if (p.balance > 0) card += '⚠️ Долг к доплате: ' + Number(p.balance).toLocaleString() + ' тг\n';
  else if (isClosed) card += '✅ Оплачено полностью\n';
  else card += '💵 Долг: уточнить\n';
  card += '💳 ' + p.channel + '\n';
  return card;
}
...
function generateShiftHTML(data) {
  ...
  return `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Отчёт смены — ${sellerName}</title>
...
function getSellerPrompt(sellerName, shopName) {
  ...
  return 'Ты — Томи, AI-управляющая магазина NANE PARIS (Астана). Ты — женщина.\n' +
    ...
    'Система проверит расстояние. Если не в магазине — геолокация не принята.\n' +
    ...
    'Система проверит расстояние. Когда получишь "Геолокация принята (закрытие)" => SHIFT_CLOSE с данными.\n\n' +
  ...
  if (reply.includes('PREPAY_LIST:')) {
    ...
    if (list.length === 0) {
      ...
      for (let i = 0; i < list.length; i += 8) {
        ...
        list.slice(i, i + 8).forEach((p, idx) => { msg += '\n' + formatPrepayCard(p, i + idx + 1); });
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (message.location) {
      ...
      if (distance > SHOP_RADIUS) {
        if (senderName) {
          ...
          for (const ownerId of OWNER_IDS) {
            await sendTelegram(ownerId, '⚠️ Попытка не из магазина!\n👤 ' + senderName + '\n📍 ' + distance + ' м от магазина\n🕐 ' + getTime());
          }
        }
        return;
      }

      // Определяем контекст — открытие или закрытие
    ...
    // Определяем контекст геолокации по тексту
    if (messageText) {
...
app.get('/', (req, res) => res.json({ status: 'ok', service: 'TOMI NANE PARIS Telegram', version: '3.5' }));
