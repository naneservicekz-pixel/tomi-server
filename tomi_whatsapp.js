...
function getSellerPrompt(sellerName, shopName, hasOpenShift, isSecondSeller, firstSellerName) {
  ...
  if (isSecondSeller) {
    return 'Ты — Томи, AI-управляющая NANE PARIS (Астана). Ты — женщина.\n' +
      ...
      'После получения геолокации с пометкой "ОТКРЫТИЯ" => SECOND_ARRIVE:{"seller":"' + sellerName + '","time":"' + now + '"}\n' +
      ...
      'После получения геолокации с пометкой "ЗАКРЫТИЯ" => SECOND_LEAVE:{"seller":"' + sellerName + '","time":"' + now + '"}\n' +
  ...
  return 'Ты — Томи, AI-управляющая NANE PARIS (Астана). Ты — женщина.\n' +
    ...
    'После получения геолокации с пометкой "ОТКРЫТИЯ" — переходи к шагу 2. НЕ говори про метки и технические детали.\n\n' +
    'ШАГ 2 — КАССА\n' +
    ...
    'После получения геолокации с пометкой "ЗАКРЫТИЯ" — передай все данные включая объяснения расхождений в notes:\n' +
  ...
  try {
    ...
    if (message.location) {
      ...
      if (action === 'open_shift') {
        // Очищаем историю перед передачей геолокации — чтобы Claude не путал с закрытием
        delete conversations[String(userId)];
        await handleMessage(userId, 'Геолокация для ОТКРЫТИЯ смены принята. Продавец в магазине. Расстояние: ' + distance + ' м. Это шаг 1 открытия смены — переходи к шагу 2, спроси наличные в кассе.', null);
      } else {
        await handleMessage(userId, 'Геолокация для ЗАКРЫТИЯ смены принята. Продавец в магазине. Расстояние: ' + distance + ' м.', null);
      }
      return;
    }
